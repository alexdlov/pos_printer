...
HANDLE PrintManager::_hPrinter = INVALID_HANDLE_VALUE;
...
{
    // Close any previously opened printer
...
BOOL PrintManager::printBytes(std::vector<uint8_t> data)
{
    ...
    docInfo.pDocName = L"POS Print Job";
    ...
    if (dwJob > 0)
    {
        ...
        // Inform the spooler that the document has ended
...
BOOL PrintManager::close()
{
    if (_hPrinter != INVALID_HANDLE_VALUE)
    {
        BOOL result = ClosePrinter(_hPrinter);
        _hPrinter = INVALID_HANDLE_VALUE;
        return result;
    }
